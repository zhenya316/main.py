...
Метод log_in, который принимает на вход аргументы: nickname, password и пытается найти пользователя в users с такими же логином и паролем. Если такой пользователь существует,
    ...
    def log_in(self, nickname, password):
        for user in self.users:
            if nickname == user.nickname:
                if int(password) == hash(user.password):
